...
public class DeleteLocation extends Subcommand {
	...
	protected void perform() {
		...
		if (args.length >= 2) {
			...
			if (!rm) {
				ActionBar.sendMessage(getPlayer(), ChatColor.RED + ErrorMessage.LOCATION_DELETE_ERROR);
				return;
			}
			ActionBar.sendMessage(getPlayer(), ChatColor.GREEN + MessageFormat.format(SuccessMessage.LOCATION_REMOVED, toRemove));
			return;
		}
		...
		ActionBar.sendMessage(getPlayer(), ChatColor.GREEN + (MessageFormat.format(SuccessMessage.LOCATION_REMOVED, locationName)));
	}
